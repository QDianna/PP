...
;pref1 = preferinte persoane ACELASI GEN
;pref2 = preferinte persoane GEN OPUS
;(car eng) = persoana de GEN OPUS
;(cdr eng) = persoana de ACELASI GEN
;get-partner
;(car eng) = persoana de ACELASI GEN

  (define (create-single-list single eng)
    (cond
      ((null? eng) single)
      ((equal? (car (car eng)) #f) (create-single-list (cons (cdr (car eng)) single) (cdr eng)))
      ((equal? (cdr (car eng)) #f) (create-single-list (cons (car (car eng)) single) (cdr eng)))
      (else (create-single-list single (cdr eng)))))

(define (match person engagements pref1 pref2 queue)
  (match-helper person engagements pref1 pref2 queue (list person)))

(define (match-helper person engagements pref1 pref2 queue single-list)
  (if (null? single-list)
      engagements
      
      (let find-partner (
                         (person-pref (get-pref-list pref1 person)))
        (cond
          ((and (null? person-pref) (null? (cdr single-list))) (cons (cons #f person) engagements))
          ((null? person-pref) (match-helper (car (cdr single-list)) (cons (cons #f person) engagements) pref1 pref2 queue (cdr single-list)))
          (else
           (let* (
                  (posible-partner (car person-pref))
                  (posible-partner-pref (get-pref-list pref2 posible-partner))
                  (posible-partner-partner (get-partner engagements posible-partner)))
             (cond
               ((not (member posible-partner (get-couple-members engagements))) (find-partner (cdr person-pref)))
               ((equal? #f posible-partner-partner) (update-engagements engagements posible-partner person))
               ((preferable? posible-partner-pref person posible-partner-partner) (match-helper posible-partner-partner (update-engagements engagements posible-partner person) pref1 pref2 queue (cons posible-partner-partner (cdr single-list))))
               (else (find-partner (cdr person-pref))))))))))

;(match 'ana '((bobo . cora) (adi . bia)) women-preferences men-preferences '(cos)) ; ⇒ '((#f . bia) (bobo . cora) (adi . ana))
...
;pref1 = preferinte persoane ACELASI GEN, pref2 = preferinte persoane GEN OPUS
;(car eng) = persoana de GEN OPUS, (cdr eng) = persoana de ACELASI GEN
;(eng-for-m engagements)
;(eng-for-w (map (λ (pair) (cons (cdr pair) (car pair))) engagements))
...
(define (path-to-stability engagements mpref wpref queue)
  (if (null? queue)
      ...
      (let (
        ...
        (if (member person (get-women wpref))
            (path-to-stability (map (λ (pair) (cons (cdr pair) (car pair))) (match person (map (λ (pair) (cons (cdr pair) (car pair))) engagements) wpref mpref queue)) mpref wpref (cdr queue))
            (path-to-stability (match person engagements mpref wpref queue) mpref wpref (cdr queue))))))
           
;(path-to-stability '((cora . bobo) (bia . adi)) men-preferences women-preferences '(ana cos)) ; ⇒ '((bia . cos) (cora . bobo) (ana . adi))
...
(define (update-stable-match engagements mpref wpref)
  (let*(
       ...
       (room-engagements (subtract engagements unstable))
