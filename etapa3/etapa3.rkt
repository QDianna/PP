...
(define (get-unstable-couples engagements mpref wpref)
  ; named let, parcurg cuplurile din engagements
  (let unstable-couples ((e engagements)
    ...
    (if (null? e)
        ...
        (let* ((couple (car e))
               (f (car couple))
               (m (cdr couple)))
          ; let*, extrag cuplul, femeia si barbatul
          (if (or (better-match-exists? m f (get-pref-list mpref m) wpref engagements) (better-match-exists? f m (get-pref-list wpref f) mpref (map (λ (pair) (cons (cdr pair) (car pair))) engagements)))
              ; daca exista un better match pentru vreunul dintre parteneri, cuplul instabil
              ; este adaugat in lista uc (unstable couples)
...
(define (engage free-men engagements mpref wpref)
  (let make-couple ((free-men-list free-men)
    ...
    ; named let, parcurg lista de barbati free-men-list
    (if (null? free-men-list)
        engagements-list
        (let* ((free-man (car free-men-list)))
          ; let*, extrag primul barbat din lista free-men
          (let find-wife ((free-man-pref (get-pref-list mpref free-man)))
            ; named let, parcurg lista de preferinte ale barbatului extras anterior
            ; si caut o posibila partenera
            (let* ((woman (car free-man-pref))
                   (woman-partner (get-partner engagements-list woman))
                   (woman-pref (get-pref-list wpref woman)))
              ; extrag prima femeie din preferinte, partenerul si preferintele acesteia
              (if (equal? #f woman-partner)
                  ; daca femeia nu are partener, formez un cuplu nou si reapelez named let-ul
                  ; 'make-couple' pentru restul barbatilor liberi din lista
                  (make-couple (cdr free-men-list) (cons (cons woman free-man) engagements-list))
                  (if (preferable? woman-pref free-man woman-partner)
                      ; daca femeia prefera barbatul liber in detrimentul partenerului actual,
                      ; formez un cuplu nou, adaug fostul partener in lista 'free-men-list' si
                      ; reapelez named let-ul make-couple pentru restul barbatilor din lista
                      (make-couple (cons woman-partner (cdr free-men-list)) (update-engagements engagements-list woman free-man))
                      ; daca femeia isi prefera partenerul actual in detrimentul barbatului
                      ; liber, reapelez named let-ul 'find-wife' pentru restul listei de preferinte
                      ; ale barbatului liber
