...
(define (get-pref-list pref person)
  (cdr (car (filter (λ (p) (equal? (car p) person)) pref))))
...
(define (preferable? pref-list x y)
  (and (not (equal? x y)) (list? (member y (member x pref-list)))))
...
(define (get-partner engagements person)
  (define lookup
    (find-first  (λ (cuplu) (equal? (car cuplu) person)) engagements))
  (if (pair? lookup)
      (cdr lookup)
      #f))
...
(define (change-first p L val)
  (cond
    ((null? L) '())
    ((p (car L)) (cons val (cdr L)))  ; returneaza val ++ restul listei fara primul element (car L)
    (else (cons (car L) (change-first p (cdr L) val)))))  ; reconstruiesc lista pana intalnesc primul element din L care satisface p
...
(define (update-engagements engagements p1 p2)
  (change-first (λ (x) (equal? (cdr x) (get-partner engagements p1))) engagements (cons p1 p2)))
...
(define (stable-match? engagements mpref wpref)
  (define all-engagements engagements)
  (null? (filter (λ(pair) (better-match-exists? (cdr pair) (car pair) (get-pref-list mpref (cdr pair)) wpref all-engagements)) engagements)))
